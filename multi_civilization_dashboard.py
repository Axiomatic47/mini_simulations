...
@app.route('/')
def serve_dashboard():
    """Serve the React dashboard HTML page (primary implementation)."""
    try:
        logger.info("Serving dashboard HTML (React implementation)")
...
def serve_dashboard_js():
    """Serve the React dashboard JavaScript file (primary implementation)."""
    try:
        logger.info("Serving React dashboard.js")
...
def serve_chart_dashboard_js():
    ...
    logger.info(f"Attempting to serve chart-dashboard.js from {dashboard_dir}")
    return send_from_directory(dashboard_dir, 'chart-dashboard.js')
...
def get_civilization_comparison():
    ...
    try:
        ...
        if request.args.get('time'):
            ...
            time_point = stats_df['Time'].max()  # Use latest available time
        ...
        for i in range(int(civ_count)):
            ...
            civs_data.append({
                ...
                'intelligence': max(0, np.random.normal(10, 3)),
                'resources': max(0, np.random.normal(50, 15)),
                'influence': max(0, np.random.normal(5, 1.5)),
            })
        ...
        result = {
            ...
            'metadata': {
                'total_civilizations': civ_count,
                'mean_value': np.mean([civ[metric] for civ in civs_data]),
                'max_value': np.max([civ[metric] for civ in civs_data]),
                'min_value': np.min([civ[metric] for civ in civs_data])
            }
        }
