I'll update the Equation Validation README with additional information about the validation process and provide the full document.
...
./run_validation.py --modules config.equations config.quantum_em_extensions
...
## Adding Physics Domain and Scale Level Tags

To help the validation framework correctly categorize your equations, add docstring annotations:
...
Bridge functions are crucial for connecting different scale levels. Each bridge function should:

1. Accept inputs from one scale level
2. Transform or translate these inputs to another scale level
3. Include docstring annotations for both scale levels

Example:
```python
def quantum_to_agent_bridge(quantum_state, agent_knowledge):
    """
...
- **Detection Issues**: Make sure your docstrings follow the expected format for physics domain and scale level annotations
- **Import Problems**: Check that the validator can properly import your modules
...
If the automated validation has trouble detecting your equations, maintain a manual mapping document:

```bash
documentation/equation_map/EQUATIONS_MAP.md
```

This can serve as a reference for your equation organization regardless of validation results.